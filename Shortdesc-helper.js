...
window.sdh.initMessages = function () {
	...
	var enwikiMessages = {
		...
		/* Popup text */
		'sdh-no-description-popup': 'The description is intentionally empty. See <a href = "wiki/Wikipedia:Short_description#SDNONE">this page</a> for more info.',
		...
		'sdh-summary-none-addition': 'that is [[WP:SDNONE|intentionally blank]]',
		/* Failure message */
...
window.sdh.main = function () {
	...
	var onResponses = function ( response, responseWD ) {
		...
		var editDescription = function ( newDescription ) {
			...
			// FIXME doesn't work with the various messages
			if ( newDescription === 'none' ) {
				summary = summary = mw.message(
					summaryMsg,
					quotify( pageDescription ),
					mw.message( 'sdh-summary-none-addition' )
				).plain() + mw.message( 'sdh-summary-append' ).plain();
			} else {
				summary = mw.message(
					summaryMsg,
					quotify( pageDescription ),
